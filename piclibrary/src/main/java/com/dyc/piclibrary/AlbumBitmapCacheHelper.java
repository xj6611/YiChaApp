...
//        cr = AppContext.getInstance().getContentResolver();
    ...
    public static AlbumBitmapCacheHelper getInstance(Context context) {
        if (instance == null) {
            synchronized (AlbumBitmapCacheHelper.class) {
                if (instance == null) {
                    instance = new AlbumBitmapCacheHelper(context);
                }
            }
        }
        return instance;
        ...
        tpe.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (width == 0 || height == 0) {
                    ...
                    File file = new File(CommonUtil.getDataPath(context));
                    if (!file.exists())
                        file.mkdirs();
                    //临时文件的文件名
                    String tempPath = CommonUtil.getDataPath(context) + hash + ".temp";
